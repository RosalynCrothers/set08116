...
void divide_triangle(const vector<vec3> &points, unsigned int count, vector<vec3> &positions, vector<vec4> &colours) {
  ...
  // IF we have more divisions to do?
	if(count > 0)
	{
		...
		glm::vec3 temp1 = (points[0] + points[1]) / 2.0f;
		glm::vec3 temp2 = (points[1] + points[2]) / 2.0f;
		glm::vec3 temp3 = (points[2] + points[0]) / 2.0f;
		// Divide new triangles

		divide_triangle({points[0],temp1, temp3}, count-1, positions, colours);
		divide_triangle({ temp1, points[1], temp2 }, count-1, positions, colours);
		divide_triangle({ temp3, temp2, points[2] }, count-1, positions, colours);
...
bool load_content() {
  ...
  vector<vec4> colours;
  divide_triangle({vec3(1.0f, -1.0f, 0.0f), vec3(0.0f, 1.0f, 0.0f), vec3(-1.0f, -1.0f, 0.0f)}, 6, positions, colours);
