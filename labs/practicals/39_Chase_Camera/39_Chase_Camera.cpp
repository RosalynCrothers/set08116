...
bool update(float delta_time) {
  ...
  if (glfwGetKey(renderer::get_window(), GLFW_KEY_UP))
	  target_mesh.get_transform().rotate(vec3(pi<float>()*delta_time, 0.0f, 0.0f));

  if (glfwGetKey(renderer::get_window(), GLFW_KEY_DOWN))
	  target_mesh.get_transform().rotate(vec3(-pi<float>()*delta_time, 0.0f, 0.0f));

  if (glfwGetKey(renderer::get_window(), GLFW_KEY_LEFT))
	  target_mesh.get_transform().rotate(vec3(0.0f, 0.0f, pi<float>()*delta_time));
...
bool render() {
  ...
  for (auto &e : meshes) {
	...
	std::cout << eff.get_uniform_location("tex") << std::endl;
